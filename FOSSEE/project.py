...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.menuOptions.setTitle(_translate("MainWindow", "Choices"))
        self.actionAdd_New_Steel.setText(_translate("MainWindow", "Add Data"))
        self.actionOpen_Steel.setText(_translate("MainWindow", "Open Data"))
        self.actionClear_Up.setText(_translate("MainWindow", "Clear"))
    ...
    def addsteel(self):
        ...
        id_chosen, ok=QtWidgets.QInputDialog.getItem(MainWindow,steel_chosen,'Choose an "ID" to be added',list,0,False)
        if ok and id_chosen:
            a=1   
        else:
            return
        ...
        if steel_chosen=='Beams':
            if int(float(id_chosen)) not in ID_list:
                ...
                try:
                    cur=conn.execute(sql)
                    self.showdlg("New Data Entry added into Beams sections.")
                    conn.commit()
                except:
                ...
                self.showdlg("Data with ID already exits.")
                return
        ...
        if steel_chosen=='Angles':
            ...
            if int(float(id_chosen)) not in ID_list:
                ...
                try:
                    cur=conn.execute(sql)
                    self.showdlg("New Data Entry added into Angles sections.")
                    conn.commit()
                except:
                    self.showdlg("Error in Operation")
                    conn.rollback()
        ...
        if steel_chosen=='Channels':
            if int(float(id_chosen)) not in ID_list:
                ...
                cur=conn.execute(sql)
                self.showdlg("New Data Entry added into Channels sections.")
                conn.commit()
                '''
                    self.showdlg("Error in Operation")
                    conn.rollback()
    ...
    def exit(self):
        import sys
        self.showdlg("Exiting from the application.")
        sys.exit()
